...
async def custom_request_logger(request: Request, call_next):
    ...
    if request.state.api_ai_call:
        async def get_model_from_request(request: Request) -> str | None:
            """Extract 'model' field from request body and restore it."""
            ...
            # Restore body for downstream handlers (IMPORTANT!)
            async def receive():
                return {"type": "http.request", "body": body}

            request._receive = receive

            # Parse and extract model
            try:
                ...
                if "application/json" in content_type:
                    ...
                    return data.get("model", [None])[0]

                elif "multipart/form-data" in content_type:
                    # For multipart, you'd need more complex parsing
                    # Usually not needed for API requests
                    pass
        ...
        def find_usage(obj: Any) -> dict | None:
            """Recursively search for 'usage' field in nested dict/list"""
        ...
        def extract_usage_from_text(text: str, usage_tokens: dict):
            """Try to extract usage from raw text (JSON or SSE)"""
            # Try parse as plain JSON first
            try:
                obj = json.loads(text)
        ...
        if 200 <= response.status_code < 400 and hasattr(response, "body_iterator"):
            # ── Case 1: StreamingResponse ──────────────────────────────────────
            ...
            if response.headers.get("content-type", "").startswith(
                ...
                async def new_iterator():
                    ...
                    async for chunk in original_iterator:
                        ...
                        buffer += text
                        usage_tokens = extract_usage_from_text(buffer, usage_tokens)
                        yield chunk
                    logs_api_call(usage_tokens)
                ...
                # Restore iterator so response still works
                async def restore_iterator():
                    yield body

                response.body_iterator = restore_iterator()

                def decompress_body(body: bytes, response) -> str:
                    content_encoding = ""
                    content_type = ""
                    ...
                    # e.g. "application/json; charset=latin-1"
                    charset = extract_charset(content_type)

                    # Try detected charset first, then fallbacks
                    encodings = [charset, "utf-8", "latin-1", "cp1252", "iso-8859-1"]
                    encodings = [e for e in encodings if e]  # Remove None/empty
                ...
                try:
                    text = decompress_body(body, response)
                    usage_tokens = extract_usage_from_text(text, usage_tokens)
                    logs_api_call(usage_tokens)
                except:
                    pass
...
async def chat_completion(
    ...
    try:
        ...
        metadata = {
            ...
            "message_attached_files": form_data.pop("message_attached_files", None),
