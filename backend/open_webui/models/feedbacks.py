...
class FeedbackTable:
    ...
    # TODO Renesas
    async def get_all_feedbacks_with_user(
        ...
        async with get_async_db_context(db) as db:
            ...
            return [
                FeedbackUserResponse(
                    **FeedbackModel.model_validate(feedback).model_dump(),
                    user=UserModel.model_validate(user) if user else None,
                )
                for feedback, user in rows
            ]

    async def get_all_feedbacks_with_user_pagination(
        self,
        ...
        async with get_async_db_context(db) as db:
            ...
            result = await db.execute(
                select(Feedback, User, snapshot_chat_title)
                .outerjoin(User, Feedback.user_id == User.id)
                .order_by(Feedback.updated_at.desc())
                .limit(limit)
                .offset(offset)
            ...
            return FeedbackUserPaginationResponse.model_validate(
                {
                    ...
                    'list': [
                        ...
                        for feedback, user, chat_title in rows
                    ],
                }
            )

    async def get_feedbacks_grouped_by_model_id(
        self, db: Optional[AsyncSession] = None
    ) -> list[FeedbackEvaluate]:
        async with get_async_db_context(db) as db:
            model_id_expr = func.json_extract(Feedback.data, '$.model_id')
            positive_expr = func.sum(
                case(
                    (cast(func.json_extract(Feedback.data, '$.rating'), BigInteger) > 0, 1),
                    else_=0,
                )
            ).label('possible')
            negative_expr = func.sum(
                case(
                    (cast(func.json_extract(Feedback.data, '$.rating'), BigInteger) < 0, 1),
                    else_=0,
                )
            ...
            result = await db.execute(
                select(model_id_expr.label('model_id'), positive_expr, negative_expr).group_by(model_id_expr)
            )
            rows = result.all()
            ...
            return [
                FeedbackEvaluate(
                    **{
                        ...
                        'count': (row.possible or 0) + (row.negative or 0),
